...
def add_galaxy_element(db, add_galaxy_cluster_body):
    add_galaxy_element = GalaxyElement(galaxy_cluster_id=add_galaxy_cluster_body.id, key="refs", value="http://github.com")
...
def test_import_galaxy_cluster_valid_data(site_admin_user_token, galaxy, organisation, tag) -> None:
...
def test_get_existing_galaxy_details(db: Session, site_admin_user_token, galaxy, add_galaxy_cluster_body, add_galaxy_element) -> None:
    galaxy_id = galaxy.id
...
def test_delete_existing_galaxy(db: Session, site_admin_user_token, galaxy, organisation, tag, add_galaxy_cluster_body, add_galaxy_element) -> None:
    galaxy_id = galaxy.id
...
def test_get_all_galaxies(db: Session, site_admin_user_token, add_galaxy_cluster_body, add_galaxy_cluster_body2) -> None:
    headers = {"authorization": site_admin_user_token}
...
def test_export_existing_galaxy(db: Session,
                                site_admin_user_token,
                                galaxy, organisation,
                                tag, add_galaxy_cluster_body, add_galaxy_cluster_body2, add_galaxy_element) -> None:
    galaxy_id = galaxy.id
...
def test_export_non_existing_galaxy(db: Session, site_admin_user_token, galaxy, organisation, tag, add_galaxy_cluster_body, add_galaxy_cluster_body2, add_galaxy_element) -> None:
    org_id = organisation.id
    tag_name = tag.name
    galaxy_id = galaxy.id
